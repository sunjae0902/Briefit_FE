...
import { convertURLToFile } from "../image/convertURLToFile";
...
export async function setUserInfoToStore() {
  try {
    ...
    const { setNickname, setProfileUrl, setProfileImageFile, setCategories } =
      useUserStore.getState();
    if (userInfo) {
      useAuthStore.setState({ isNew: false }); // 로컬 스토리지 상태 업데이트
      setNickname(userInfo.nickname);
      setProfileUrl(userInfo.profileUrl);
      setProfileImageFile(await convertURLToFile(userInfo.profileUrl));
